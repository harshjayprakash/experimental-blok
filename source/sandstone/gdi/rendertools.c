...
int stRenderToolsInit(TRenderTools *pTools, const TThemePalette *pColours)
{
    ...
    pTools->brushes.hBaseBackgroundFaded =
        CreateSolidBrush(pColours->baseBackgroundFaded);
    pTools->brushes.hBaseBackgroundMedium =
        CreateSolidBrush(pColours->baseBackgroundMedium);
    pTools->brushes.hPrimaryBackground = CreateSolidBrush(pColours->primaryBackground);
    pTools->brushes.hPrimaryBackgroundFaded =
        CreateSolidBrush(pColours->primaryBackgroundFaded);
    ...
    int penCheck =
        (pTools->pens.hBaseForeground != NULL && pTools->pens.hBaseBorder != NULL &&
         pTools->pens.hBaseBorderFaded != NULL && pTools->pens.hPrimaryBorder != NULL &&
